...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-900 text-gray-200 font-sans overflow-hidden relative">
      <div
      ...
      >
        ...
        <footer className="w-full p-3 text-center text-xs text-gray-500 border-t border-gray-700 mt-8">
          © 01 04 2025 GitBub Inc. | Not affiliated with GitHub | Terms |
          Privacy (lol)
        </footer>
      </div>
      ...
      <div
        className={`absolute inset-0 w-full h-full flex items-center justify-center bg-black transition-opacity duration-500 ${
          showVideo ? "opacity-100 z-20" : "opacity-0 pointer-events-none -z-20"
        }`}
      >
        <video
          ref={videoRef}
        ...
        {showFinalMessage && (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-30 bg-black bg-opacity-60">
            <Confetti
            ...
            <button
              ...
              className="mt-8 bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded z-40"
            >
              LOL, ещё разок?
            </button>
          </div>
